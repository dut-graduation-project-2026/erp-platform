...
import { Plus, Search, Building2, Phone, Mail, X, Save } from 'lucide-react';
...
export default function CustomersListPage({ params }: { params: Promise<{ orgId: string }> }) {
  ...
  const handleOpenModal = (partner?: SalePartner) => {
    if (partner) {
      ...
      setSelectedPartner({ name: '', code: '', email: '', phone: '', address: '', type: 'CUSTOMER' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSavePartner = async () => {
    ...
    if (!selectedPartner?.code) return alert('Partner code is required.');
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-[8px] shadow-[0px_12px_28px_rgba(0,0,0,0.30)] w-full max-w-[600px] flex flex-col max-h-[90vh]">
             ...
             <div className="p-6 overflow-y-auto space-y-5">
               <div className="grid grid-cols-2 gap-6">
                 ...
                 <div>
                   ...
                   <select 
                     value={selectedPartner.type || 'CUSTOMER'}
                     onChange={e => setSelectedPartner({...selectedPartner, type: e.target.value as 'CUSTOMER' | 'VENDOR'})}
             ...
             <div className="px-6 py-4 bg-[#f8f8f8] border-t border-[#e0e0e0] flex justify-end space-x-2 shrink-0">
                ...
                {(selectedPartner.id ? hasPermission('sales:write') : hasPermission('sales:create')) && (
                  <Button 
