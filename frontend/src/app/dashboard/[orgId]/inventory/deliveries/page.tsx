...
import { Search, RefreshCw, Eye, Calendar, ArrowRightLeft } from 'lucide-react';
...
export default function DeliveriesPage() {
  ...
  const fetchDocuments = () => {
    ...
    getInventoryDocuments(orgId, selectedWarehouseId, {
      ...
      .then(res => {
        // Filter only ISSUES (Deliveries)
        const issues = (res.data || []).filter(d => d.documentType === DOCUMENT_TYPE.ISSUE);
        setDocuments(issues);
      })
      .catch(err => {
        console.error(err);
    ...
    <div className="p-6 h-full flex flex-col font-['Segoe_UI'] bg-white">
      ...
      <div className="flex justify-between items-center mb-5 shrink-0">
        <div className="flex items-center">
          <ArrowRightLeft className="w-6 h-6 mr-3 text-[#0066cc]" />
          <div>
            <h1 className="text-[24px] font-[600] text-[#242424] mb-1">Delivery Orders</h1>
            <span className="text-[14px] text-[#898989]">Manage outbound stock issues</span>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white border-b border-[#e0e0e0]">
                ...
                <th className="py-3 px-4 text-[12px] font-bold text-[#242424] uppercase tracking-wider">Customer</th>
                <th className="py-3 px-4 text-[12px] font-bold text-[#242424] uppercase tracking-wider">Delivery Address</th>
                ...
                filteredDocs.map((doc) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-[13px] text-[#242424] font-medium">
                        {doc.partnerName || <span className="text-[#898989] italic">N/A</span>}
                      </td>
                      <td className="py-3.5 px-4 text-[13px] text-[#64748b] max-w-[200px] truncate" title={doc.deliveryAddress || ''}>
                        {doc.deliveryAddress || <span className="text-[#898989] italic">No address</span>}
                      </td>
                      <td className="py-3.5 px-4 text-[13px] text-[#4a4a4a] font-medium">
                        {doc.referenceType !== 'MANUAL' ? (
                          ...
                          <span className="text-[#898989] italic">Manual delivery</span>
                        )}
                      </td>
