...
export default function NewReceiptPage({ 
  ...
  useEffect(() => {
    ...
    ]).then(async ([whRes, prodRes]) => {
      const whs = whRes.data || [];
      setWarehouses(whs);
      const whId = defaultWarehouseId || (whs.length > 0 ? whs[0].id : '');
      if (!warehouseId) {
        setWarehouseId(whId);
      }
      setProducts(prodRes.data || []);

      // Auto-fill logic for shortages
      if (fulfillShortageFor && whId) {
        try {
          const doc = await getInventoryDocumentById(orgId, whId, fulfillShortageFor);
          const balances = await getInventoryBalances(orgId, whId, { limit: 500 });
          const stockMap: Record<string, number> = {};
          balances.data?.forEach(b => {
            stockMap[b.product.id] = b.quantity;
          });

          const shortages: { productId: string; quantity: number }[] = [];
          ...
          if (shortages.length > 0) {
            setItems(shortages);
            setNotes(`Replenishing shortages for document: ${doc.name}`);
            toast.info(`Auto-filled ${shortages.length} missing products.`);
          }
        } catch (e) {
          console.error('Failed to auto-fill shortages', e);
        }
      }
    }).catch((err) => {
      console.error(err);
  ...
  }, [orgId, defaultWarehouseId, fulfillShortageFor]);
      ...
      <div className="px-6 py-4 border-b border-[#e0e0e0] flex items-center justify-between shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2 text-[#242424]">
            ...
            <h2 className="text-[18px] font-[600]">New Receipt</h2>
          </div>
        </div>
        ...
        <div className="max-w-[1000px] mx-auto space-y-6">
          ...
          <div className="bg-white border border-[#e0e0e0] rounded-[6px] shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <label className="block text-[13px] font-[600] text-[#242424] mb-1.5">Destination Warehouse *</label>
                <select
                  value={warehouseId}
            ...
            <div className="mt-4">
              ...
              <Textarea 
                ...
                placeholder="Internal notes or supplier reference..."
                rows={3}
                className="text-[13px]"
              />
            </div>
          ...
          <div className="bg-white border border-[#e0e0e0] rounded-[6px] shadow-sm p-6">
            <div className="flex justify-between items-center mb-4 border-b border-[#f0f0f0] pb-2">
              <h3 className="text-[14px] font-[700] text-[#242424]">Products Received</h3>
              <Button 
