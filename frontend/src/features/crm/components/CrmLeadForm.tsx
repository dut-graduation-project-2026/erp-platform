...
import { Calendar, Clock, MessageSquare, Mail, Phone, Building } from 'lucide-react';
...
export function CrmLeadForm({ lead, orgId, isNew = false }: Props) {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      if (formData.stageId && formData.stageId.length === 36) {
        if (isNew) {
           const res = await createLead(orgId, formData as any);
           router.push(`/dashboard/${orgId}/crm/leads/${res.id}`);
        } else if (lead?.id) {
           await updateLead(orgId, lead.id, formData as any);
           alert("Lead updated successfully!");
        }
      } else {
        await new Promise(resolve => setTimeout(resolve, 500));
        if (isNew) {
           alert("Lead created successfully! (Mocked due to missing Stage UUID)");
           router.push(`/dashboard/${orgId}/crm`);
        } else {
           alert("Lead updated successfully! (Mocked due to missing Stage UUID)");
        }
      }
    } catch (e) {
      console.error(e);
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-3/5 overflow-y-auto p-6 border-r border-[#e0e0e0]">
           <div className="max-w-2xl space-y-6">
             ...
             <div>
               <label className="block text-[14px] font-[600] text-[#242424] mb-1">Opportunity</label>
               <Input 
                 value={formData.name}
             ...
             <div className="grid grid-cols-2 gap-6">
               <div>
                 <label className="block text-[14px] font-[600] text-[#242424] mb-1">Customer / Partner</label>
                 <Input 
                   readOnly
                   value={lead?.partner?.name || 'Search Partner...'}
                   className="h-10 border-[#d0d0d0] rounded-[4px] bg-[#f8f8f8] text-[#0066cc] cursor-pointer"
                 />
               </div>
               <div>
                 ...
                 <Input 
                   readOnly
                   value={lead?.salesperson ? `${lead.salesperson.firstName} ${lead.salesperson.lastName}` : 'Unassigned'}
                   className="h-10 border-[#d0d0d0] rounded-[4px] bg-[#f8f8f8]"
                 />
               </div>
             </div>
      ...
      {showConvertModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-[8px] shadow-[0px_12px_28px_rgba(0,0,0,0.30)] w-full max-w-[500px] overflow-hidden">
             <div className="p-6">
                <h2 className="text-[24px] font-[700] text-[#242424] mb-6">Convert to Sales Order</h2>
                ...
                <div className="space-y-4">
                  <label className="flex items-center space-x-3 cursor-pointer">
                    ...
                    />
                    <span className="text-[14px] text-[#242424]">Create a new quotation</span>
                  </label>
                  <label className="flex items-center space-x-3 cursor-pointer">
                    <input 
                      type="radio" 
                      name="convertType" 
                      value="link"
                      checked={convertType === 'link'}
                      onChange={(e) => setConvertType(e.target.value)}
                      className="w-4 h-4 text-[#0066cc] focus:ring-[#0066cc]"
                    />
                    <span className="text-[14px] text-[#242424]">Link to existing quotation</span>
                  </label>
                </div>
             </div>
