...
public class WarehouseServiceImpl implements WarehouseService {
  ...
  @Override
  public PagedEntityResponse<WarehouseBaseResponse> getWarehousesByOrganizationId(
    ...
    Pageable pageable =
        PageRequest.of(
            ...
            SortingConstants.customEntitiesSort(
                SortField.asc("name"), SortField.asc("code")));
  ...
  public WarehouseResponse getWarehouseById(UUID organizationId, UUID warehouseId) {
    ...
    Warehouse warehouse = findWarehouseByIdAndVerifyOrganization(warehouseId, organizationId);
  ...
  public WarehouseResponse createWarehouse(UUID organizationId, CreateWarehouseRequest request) {
    log.info("Creating warehouse with code {} in organization {}", request.code(), organizationId);
  ...
  public void deleteWarehouse(UUID organizationId, UUID warehouseId) {
    log.info("Deleting warehouse {} from organization {}", warehouseId, organizationId);
    findOrganizationById(organizationId);
    Warehouse warehouse = findWarehouseByIdAndVerifyOrganization(warehouseId, organizationId);
  ...
  private Warehouse findWarehouseByIdAndVerifyOrganization(UUID warehouseId, UUID organizationId) {
    Warehouse warehouse =
        warehouseRepository
  ...
  private void assertCodeAvailable(String code, UUID organizationId) {
    if (warehouseRepository.findByCodeAndOrganizationId(code, organizationId).isPresent()) {
      throw new ResourceAlreadyExistsException(
          "Warehouse with code '" + code + "' already exists in this organization.");
    }
  }
}
